 ...
 */
export function processPerformanceExcel(data: any[][]): ProcessingResult<ProcessedPerformance> {
  ...
  const rows = data.slice(1);
  let lastValidDate = new Date().toISOString().split('T')[0]; // Default to today
