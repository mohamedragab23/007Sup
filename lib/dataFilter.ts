...
export async function getSupervisorPerformanceFiltered(
  ...
  try {
    ...
    for (let i = 1; i < allData.length; i++) {
      ...
      if (!riderCodes.has(riderCode)) continue;
    ...
    console.log(`[Performance Filter] Total rows in sheet: ${allData.length - 1}, Rider codes assigned: ${riderCodes.size}`);
    ...
    if (filtered.length === 0 && allData.length > 1) {
      ...
      console.warn(`[Performance Filter] Supervisor rider codes: ${Array.from(riderCodes).slice(0, 10).join(', ')}`);
